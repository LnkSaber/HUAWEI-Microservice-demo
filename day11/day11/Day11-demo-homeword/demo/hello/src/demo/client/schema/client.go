...
func (*Client) Greeting(ctx *restful.Context) {
	openlogging.GetLogger().Info("access consumer Greeting")
	req, err := rest.NewRequest(http.MethodPost, "cse://provider/provider/v0/greeting", nil)
	if err != nil {
		ctx.WriteError(http.StatusInternalServerError, err)
		return
	}
	req.Body = ctx.ReadRequest().Body
	resp, err := core.NewRestInvoker().ContextDo(context.TODO(), req)
	if err != nil {
		ctx.WriteError(http.StatusInternalServerError, err)
		return
...
func (*Client) URLPatterns() []restful.Route {
	return []restful.Route{
		...
		{Method: http.MethodPost, Path: "/consumer/v0/greeting", ResourceFuncName: "Greeting"},
	}
}
